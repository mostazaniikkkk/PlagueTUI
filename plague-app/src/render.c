...
void pa_do_render(int cols, int rows, int css_handle)
{
    ...
    /* 4. Dibujar cada widget (orden 0..N → padre antes que hijo) */
    for (int i = 0; i < g_widget_count; i++) {
        PA_Widget *w = &g_widgets[i];
        ...
        if (css_handle > 0)
            style = pc_compute_style(css_handle, w->type, w->id, w->classes, 0);
        ...
        if (w->text[0] != '\0') {
            ...
            /* Posición del texto: top-left del content_region
             * (centrado vertical solo si es una sola línea) */
            TG_Region cr = pl_tree_result(i).content_region;
            int text_x = cr.x;
            int has_nl = (strchr(w->text, '\n') != NULL);
            int text_y = has_nl ? cr.y : (cr.y + cr.height / 2);
